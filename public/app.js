...
const state={view:'campaign',file:null,campaign:null,campaignSig:'',expanded:null,selectedRows:new Set(),selectionAnchorRow:null,previews:new Map(),poll:null,pollBusy:false,eventSource:null,eventReconnect:null,eventCampaignId:'',verification:new Map(),bulkSend:{active:false,total:0,sent:0,failed:0,currentRow:null,nextDelayMs:0,message:'',stopRequested:false},mailSenders:{loading:false,error:'',list:[],selectedEmail:'',selectedName:'',defaultEmail:'',defaultName:'',bulkUsageByDomain:{},bulkUsageBySender:{}},blocklist:[]};
...
function setView(v){state.view=v;navBtns.forEach(b=>b.classList.toggle('active',b.dataset.view===v));mailerView.classList.toggle('active-view',v==='mailer');campaignView.classList.toggle('active-view',v==='campaign');if(warmupView)warmupView.classList.toggle('active-view',v==='warmup');if(noMailView)noMailView.classList.toggle('active-view',v==='nomail');if(v==='mailer'){breadcrumb.textContent='Cold Mailbot / The Mailer';pageTitle.textContent='Content Console';}else if(v==='warmup'){breadcrumb.textContent='Cold Mailbot / Warmup';pageTitle.textContent='Email Warmup';}else if(v==='nomail'){breadcrumb.textContent='Cold Mailbot / No Mail';pageTitle.textContent='Blocklist';}else{breadcrumb.textContent='Cold Mailbot / Campaign';pageTitle.textContent='Run Console';}}
...
// ── Blocklist ──────────────────────────────────────────────
function isEmailBlocked(email){const e=lower(email);if(!e)return false;return state.blocklist.some(item=>lower(item.email)===e);}
function getBlocklistEntry(email){const e=lower(email);return state.blocklist.find(item=>lower(item.email)===e)||null;}
function renderNoMailView(){const list=state.blocklist;const countEl=$('#noMailCount');if(countEl)countEl.textContent=String(list.length);const listEl=$('#noMailList');if(!listEl)return;if(!list.length){listEl.innerHTML='<div class="nomail-empty"><p class="mono-label">No blocked emails</p><p class="muted">Emails you add will appear here. Campaign rows matching a blocked email will show <strong>Do Not Send</strong>.</p></div>';return;}listEl.innerHTML='';list.forEach(entry=>{const row=document.createElement('div');row.className='nomail-row';const info=document.createElement('div');info.className='nomail-row-info';const emailEl=document.createElement('p');emailEl.className='nomail-row-email';emailEl.textContent=entry.email;const nameEl=document.createElement('p');nameEl.className='nomail-row-name';nameEl.textContent=entry.name||'—';const dateEl=document.createElement('p');dateEl.className='nomail-row-date';const addedTs=Date.parse(String(entry.addedAt||''));dateEl.textContent=Number.isFinite(addedTs)?`Added ${new Date(addedTs).toLocaleString()}`:'';info.append(emailEl,nameEl,dateEl);const removeBtn=document.createElement('button');removeBtn.type='button';removeBtn.className='nomail-remove-btn';removeBtn.title=`Remove ${entry.email} from blocklist`;removeBtn.textContent='×';removeBtn.addEventListener('click',async()=>{removeBtn.disabled=true;try{const p=await api(`/api/blocklist/${encodeURIComponent(entry.email)}`,null,'DELETE');state.blocklist=Array.isArray(p?.blocklist)?p.blocklist:[];renderNoMailView();renderCampaign();}catch(e){const errEl=$('#noMailAddError');if(errEl){errEl.textContent=e.message||'Remove failed';errEl.classList.remove('hidden');setTimeout(()=>errEl.classList.add('hidden'),4000);}removeBtn.disabled=false;}});row.append(info,removeBtn);listEl.appendChild(row);});}
async function loadBlocklist(){try{const p=await api('/api/blocklist',null,'GET');state.blocklist=Array.isArray(p?.blocklist)?p.blocklist:[];renderNoMailView();}catch{state.blocklist=[];}}
function bindNoMailView(){const addBtn=$('#noMailAddBtn');const emailInput=$('#noMailEmailInput');const nameInput=$('#noMailNameInput');const errEl=$('#noMailAddError');if(!addBtn||!emailInput)return;async function doAdd(){const email=compact(emailInput.value);const name=compact(nameInput.value||'');if(!email){if(errEl){errEl.textContent='Email is required';errEl.classList.remove('hidden');}return;}addBtn.disabled=true;if(errEl)errEl.classList.add('hidden');try{const p=await api('/api/blocklist',{email,name});state.blocklist=Array.isArray(p?.blocklist)?p.blocklist:[];emailInput.value='';if(nameInput)nameInput.value='';renderNoMailView();renderCampaign();}catch(e){if(errEl){errEl.textContent=e.message||'Failed to add email';errEl.classList.remove('hidden');}}finally{addBtn.disabled=false;}}addBtn.addEventListener('click',doAdd);emailInput.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();doAdd();}});}
// ───────────────────────────────────────────────────────────
...
a.className=`queue-row${ui.blocklisted?' row-blocked':''}`;
...
tx.className=`queue-status-text${ui.blocklisted?' blocked-label':''}`;
tx.textContent=`${ui.label}${!ui.blocklisted&&Number(row.durationMs)>0?` ${fmtMs(row.durationMs)}`:''}`;
sw.append(dot,tx);
if(!ui.blocklisted&&isRowSendReady(row)){
...
a.appendChild(hd);
if(ui.blocklisted){const bm=document.createElement('div');bm.className='queue-blocked-meta';const be=ui.blocklistEntry;const nameStr=compact(be?.name);bm.innerHTML=`<span><span class="bm-label">Email</span>${lower(ui.contactEmail)||'-'}</span>${nameStr?`<span><span class="bm-label">Name</span>${nameStr}</span>`:''}<span><span class="bm-label">Status</span>Blocked — will not be sent</span>`;a.appendChild(bm);}
if(!ui.blocklisted&&exp)rowExpand(row,a);
...
async function togglePause(){if(!state.campaign?.id)return;const st=statusNorm(state.campaign.status);if(['completed','failed'].includes(st))return;const btnLabel=String(pauseResumeBtn.textContent||'').trim().toLowerCase();const ep=btnLabel==='pause'?'pause':'resume';pauseResumeBtn.disabled=true;try{const p=await api(`/api/campaigns/${encodeURIComponent(state.campaign.id)}/${ep}`,{});if(p?.campaign){setCampaign(p.campaign,{force:true});renderCampaign();}}catch(e){rawOutput.textContent=j({error:e.message||'Pause/resume failed'});}finally{pauseResumeBtn.disabled=false;}}
